...
> 코딩 몰라도 됩니다. Python 만 설치돼 있으면 돼요.

```bash
# 1) 설치 (이미 있으면 최신으로 업데이트)
pip install --upgrade "goldfish-finance[charts,xlsx]"

# 2) 작업 폴더 만들기
goldfish init
```

**3)** 생성된 `goldfish-분석` 폴더에 **거래내역 파일(CSV/엑셀)을 넣고** →
폴더 안 **`리포트_만들기.bat` 더블클릭**(Windows) → `리포트` 폴더에 HTML 리포트 완성! 🐠

- 파일 이름은 **아무거나** 괜찮습니다(증권사에서 받은 그대로 OK). 폴더 안 모든 CSV/엑셀을 분석합니다.
- 천단위 쉼표·한글 인코딩·빈 행은 **자동 정리**됩니다.
- 입력 양식이 필요하면 `_템플릿` 폴더의 CSV/엑셀을 참고하세요.

> 명령으로도 가능: `goldfish goldfish-분석` (폴더), `goldfish 파일.csv` (단일 파일)
