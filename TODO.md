...
### v1.1.1 (패치)
- [x] `리포트_만들기.bat` 버그 수정 — `python -m goldfish.cli "%~dp0"` 의 `%~dp0` 끝 역슬래시가 닫는 따옴표를 escape 해 폴더 경로가 깨지던 문제(`CSV 파일을 찾을 수 없습니다: ...분석"`). `cd /d "%~dp0"` 후 `goldfish.cli .` 로 호출하도록 변경(런처 회귀 테스트 2개 추가). 사용자 기존 .bat 도 즉시 수정.
- [x] README 상단 "🚀 빠른 시작 (3단계)" 비개발자용 가이드 추가, 보안 재점검(HTML 출력 _esc 이스케이프로 XSS 차단, 신규코드 위험패턴 0).
- [x] 버전 1.1.1 → **PyPI 배포 완료**(2026-06-23, https://pypi.org/project/goldfish-finance/1.1.1/) + `v1.1.1` 태그 푸시. 전체 검토(버전일관/67테스트/빌드/비밀파일/git clean) 통과.
